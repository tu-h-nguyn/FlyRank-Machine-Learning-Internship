...
Both pages count visits with **Google Analytics 4, `G-KGRCWRY9BV`**. That is already
stamped in and the audit passes on it; nothing here is blocking. The rest of this section
is how to change it.

One counter, one place. Everything between the `analytics:start` and `analytics:end`
comments in each page is *generated* from `site.json` — never edit it by hand, or the next
`configure_site.py` run will overwrite you. That is also why the pages cannot end up
...
that is the deliverable, not a screenshot of the script tag. GA4 can take up to 24 hours
for standard reports; look at **Realtime**, which shows the visit within about a minute.

If the dashboard stays empty, it is almost always an ad blocker — both
`googletagmanager.com` and `gc.zgo.at` are on every blocklist. Test with one off.

**One thing to know about GA4 over GoatCounter.** GA4 sets cookies and sends data to
Google, so in the EU/UK it needs a consent banner to be lawful; this page ships none. For
a personal portfolio read by recruiters that is a judgement you are making, not an
oversight — but it is the reason the alternative is kept one command away. GoatCounter is
free for personal use, about 3 KB, sets no cookies and stores no personal data, so it
needs no banner:

```bash
python3 work/scripts/configure_site.py --analytics <your-goatcounter-code>
```
