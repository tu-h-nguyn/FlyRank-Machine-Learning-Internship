...
PROVIDER = cfg.get("analytics_provider", "")
ANALYTICS_ID = cfg.get("analytics_id", "") or cfg.get("goatcounter_code", "")
...
print(f"analytics  {f'{PROVIDER} {ANALYTICS_ID}' if PROVIDER else '(unset)'}")
...
for name, path, url in PAGES:
    ...
    # --- analytics ---
    # The block is generated by configure_site.py, so the page and site.json cannot
    # drift apart: a page shipping a counter site.json does not know about, or two
    # counters at once, is a finding.
    ...
    if check("analytics block present", bool(block),
        ...
        check("analytics provider configured", bool(PROVIDER),
              f"{PROVIDER} {ANALYTICS_ID}" if PROVIDER else "unset — nothing will be counted")
        found = [name for name, (pat, _) in ANALYTICS.items() if pat.search(body)]
        if PROVIDER:
            pattern = ANALYTICS[PROVIDER][0]
            m = pattern.search(body)
            check(f"{PROVIDER} snippet stamped into the page", bool(m),
                  m.group(0) if m else "site.json says " + PROVIDER + " but the page has no such tag")
            if m:
        ...
        # A counter outside the managed block would survive every configure_site.py run.
        outside = html.replace(body, "")
        strays = [name for name, (pat, _) in ANALYTICS.items() if pat.search(outside)]
