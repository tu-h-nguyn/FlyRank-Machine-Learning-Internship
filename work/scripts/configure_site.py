...
def load():
    """Read site.json, migrating the older goatcounter_code key if it is still there."""
    cfg = json.loads(CONFIG.read_text(encoding="utf-8"))
    legacy = cfg.pop("goatcounter_code", None)
    if legacy and not cfg.get("analytics_id"):
        cfg["analytics_provider"] = "goatcounter"
        cfg["analytics_id"] = legacy
    cfg.setdefault("analytics_provider", "")
    cfg.setdefault("analytics_id", "")
    return cfg
