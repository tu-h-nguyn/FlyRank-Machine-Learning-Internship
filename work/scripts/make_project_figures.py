...
and their curves draw themselves on as the card scrolls into view.
    ...
    python3 work/scripts/make_project_figures.py            # rewrite the figures in the page sources
...
work/portfolio/site/pages/*.html is generated here; do not edit it by hand. A figure
may appear on more than one page (PINNs is on both the projects and the thesis page);
every copy is rewritten. Run build_portfolio.py afterwards to publish the pages.
"""
import argparse
import json
import math
...
PAGES = ROOT / "work" / "portfolio" / "site" / "pages"
DATA = ROOT / "work" / "portfolio" / "figure-data"

# Every colour that draws text passes WCAG AA (4.5:1) on the white plate, and still does
# when dark mode dims the plate to 90% (audit_portfolio.py checks both).
INK, NAVY, SLATE, MUTED, GRID, PAPER = "#111111", "#1c2d3d", "#627689", "#5f6469", "#e3e1dc", "#ffffff"
...
def ppa():
    ...
    pa += [line(a, ref_k, [g0 / k for k in ref_k], GRID.replace("e3e1dc", "b9b5ab"), 1.2, "3 5", cls="dfig-ref"),
           ...
           f'<text x="{a.x(40):.1f}" y="{a.y(g0 / 40) - 8:.1f}" fill="{MUTED}" font-size="13">O(1/k)</text>',
           f'<text x="{a.x(40):.1f}" y="{a.y(g0 / 1600) - 8:.1f}" fill="{MUTED}" font-size="13">O(1/k²)</text>']
...
def main():
    ...
    ap.add_argument("--check", action="store_true", help="fail if any figure in the pages is stale")
    args = ap.parse_args()

    blocks = {}
    found = {name: 0 for name in FIGURES}
    stale = []
    for path in sorted(PAGES.glob("*.html")):
        ...
        for name in FIGURES:
            ...
            pos = 0
            while (m := pat.search(new, pos)):
                if name not in blocks:
                    blocks[name] = FIGURES[name]()
                text = block(name, m.group(1), blocks[name])
                new = new[:m.start()] + text + new[m.end():]
                pos = m.start() + len(text)
                found[name] += 1
        rel = path.relative_to(ROOT)
        if new == page:
            print(f"{rel}: figures current")
            continue
        stale.append(str(rel))
        if not args.check:
            path.write_text(new, encoding="utf-8")
            print(f"{rel}: figures rewritten")

    missing = [n for n, c in found.items() if not c]
    if missing:
        sys.exit(f"no figure:NAME:start / figure:NAME:end markers for {missing} in {PAGES.relative_to(ROOT)}")
    if args.check and stale:
        sys.exit("figures are stale — run make_project_figures.py, then build_portfolio.py: " + ", ".join(stale))
